...
async def init(slave: bool) -> None:
    ...
    if config.settings.master_config:
        ...
        content: configuration.Config | configuration.SourceConfig = config.settings.master_config
    else:
    ...
    if content.get("sources", False) is not False:
        full_config = cast("configuration.Config", content)
        _LOG.info("The master config is inline")
        source_config = cast("configuration.SourceConfig", content)
        source_config["standalone"] = True
        # A fake master source to have auth work
        MASTER_SOURCE = base.BaseSource(_MASTER_ID, source_config, is_master=True)
        await _handle_master_config(full_config)
    else:
        source_config = cast("configuration.SourceConfig", content)
        MASTER_SOURCE = _create_source(_MASTER_ID, source_config, is_master=True)
