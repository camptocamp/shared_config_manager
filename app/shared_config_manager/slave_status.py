...
class GetSlavesStatusProto(Protocol):
    ...
    async def __call__(self) -> list[SlaveStatus] | None: ...


get_slaves_status: GetSlavesStatusProto = None  # type: ignore[assignment]


async def _get_slaves_status() -> SlaveStatus:
    """Get the status of all the slaves."""
    return SlaveStatus(sources=registry.get_stats())


class GetSourceStatusProto(Protocol):
    """Protocol for get_source_status function."""

    async def __call__(self, *, source_id: str) -> list[SourceStatus] | None: ...


get_source_status: GetSourceStatusProto = None  # type: ignore[assignment]


async def _get_source_status(*, source_id: str) -> SourceStatus:
    """Get the status of a source."""
    source = registry.get_source(source_id)
    if source is None:
        return SourceStatus(filtered=source_id in registry.FILTERED_SOURCES)
