...
def test_webhook(app_connection: Connection) -> None:
    answer = _trigger(
        app_connection,
        "1/refresh/test_git",
        json={
...
def test_webhook_other_branch(app_connection: Connection) -> None:
    answer = _trigger(
        app_connection,
        "1/refresh/test_git",
        json={
...
def test_webhook_not_push(app_connection: Connection) -> None:
    answer = _trigger(
        app_connection,
        "1/refresh/test_git",
        {
...
def test_all_webhook(app_connection: Connection) -> None:
    answer = _trigger(
        app_connection,
        "1/refresh",
...
def test_all_no_auth(app_connection: Connection) -> None:
    app_connection.get(
        "1/refresh",
        expected_status=302,
        allow_redirects=False,
